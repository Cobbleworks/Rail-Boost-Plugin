...
- Server version **1.16 or higher** (pi-version: 1.16 is the minimum)
...
| utopickup | 	rue/false | Toggle item auto-pickup |
| adius | 1..5 | Auto-pickup radius |
| speedometer | 	rue/false | Toggle BossBar speedometer |
| chunkload | 	rue/false | Toggle temporary force-loading of traversed chunks |
| magnet | 	rue/false | Toggle minecart magnetism behavior |
| effects | 	rue/false | Toggle particle trail effects |
| effecttype | Bukkit particle name | Set particle type (for example FLAME, END_ROD) |
| lacklist | dd/remove <material> | Add/remove material from auto-pickup blacklist |
...
|-----|------|-------------|
| 
ame | string | Display name of preset |
| speed | int | Speed level 1..6 |
| utopickup | boolean | Auto-pickup enabled flag |
...
| lacklist | list | Material names blocked from auto-pickup |
...
| collector | utopickup=true, pickupRadius=4, speed=2 |
...
|------------|-------------|---------|
| ailboost.use | Allows use of RailBoost features | 	rue |
| ailboost.admin | Administrative access to RailBoost | op |
...
The output JAR is written to 	arget/Rail-Boost-x.x.x.jar. Copy it into your server's plugins/ folder as described in the [Installation Steps](#installation-steps) section.
